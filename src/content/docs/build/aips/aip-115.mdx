...
The `0x1::account::Account` resource is not created when an address is first
used. It is created when a sequence-number transaction needs to increment the
sequence number, including a first transaction with sequence number `0`.

An [orderless transaction](/build/aips/aip-123) does not need the account
resource and does not create it.
