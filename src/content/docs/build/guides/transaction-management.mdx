...
through the system by an additional rule. If a transaction's sequence number is
...
Those pull requests show a historical sequence-number manager. For current SDK
builders, see [Building Transactions](/build/sdks/ts-sdk/building-transactions)
and [Orderless Transactions](/build/sdks/ts-sdk/building-transactions/orderless-transactions).

## When to use orderless transactions instead

Sequence numbers serialize all submissions from one account. If several machines
must submit from the same sender without coordinating a sequence-number
allocator — for example an exchange hot wallet — use
[orderless transactions](/build/guides/orderless-transactions) (AIP-123). Each
transaction carries a unique `replayProtectionNonce` and expires in at most 60
seconds. You can still use the sequence-number model above when you need strict
ordering or longer expiration.
