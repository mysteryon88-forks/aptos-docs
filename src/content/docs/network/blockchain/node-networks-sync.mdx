...
- **Archival nodes (ANs)** — a fullnode that retains blockchain data since genesis. See [Archival PFNs](/network/nodes/configure/state-sync#archival-pfns).
...
As of 2026, mainnet block times are typically tens of milliseconds. See [Blocks](/network/blockchain/blocks) and [Explorer](https://explorer.aptoslabs.com/blocks?network=mainnet) for live timestamps.
