...
| storage\_fee\_per\_state\_byte         | Notice this is charged every time the slot grows in size, not only at allocation time. (However, for simplicity, refunding is only at deletion time. See [AIP-65](https://github.com/aptos-foundation/AIPs/blob/main/aips/aip-065-storage-fee-for-state-bytes-refundable.md#specification)) |
