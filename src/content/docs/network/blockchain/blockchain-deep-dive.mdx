...
- The mempool for validator node V<sub>X</sub> accepts a sequenced transaction T<sub>N</sub> only if its sequence number is greater than or equal to the sender's current sequence number. [Orderless transactions](/build/guides/orderless-transactions) skip that contiguous-sequence check and use a nonce instead.

### 2. Mempool → Other validator nodes

- Shared mempool forwards user transactions from fullnodes toward the validator set.
- Inside the validator set, [Quorum Store](/network/glossary#quorum-store) disseminates those transactions in certified batches. Validators do not flood raw mempool contents at each other as the primary data path.

### 3. Consensus → Mempool

- When a validator becomes the leader, consensus pulls certified batches from Quorum Store and forms a proposed block. Mempool still holds the waiting transactions that Quorum Store batched.
- Inclusion in a proposed block does not guarantee that T<sub>N</sub> will persist. The block still has to collect a quorum of votes and commit.
...
When validator V<sub>X</sub> is a leader/proposer, consensus pulls certified batches from [Quorum Store](/network/glossary#quorum-store) and forms a proposed block of transactions. Mempool is the buffer those batches were built from, not the proposal payload itself.
