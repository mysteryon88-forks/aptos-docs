...
自语言版本 **2.2** 起，Move 支持**函数值**（一等函数）。函数值可以存储并像其他值一样传递。

函数值**不受重入锁定**：通过函数值的回调可以重入原模块。相比之下，可分派同质化资产的原生分派函数仍锁定，不能重入当前交易中已调用的模块。编写接受函数值的 API 时，应在调用回调之前完成状态更新，并假设回调可能再次进入本模块。
